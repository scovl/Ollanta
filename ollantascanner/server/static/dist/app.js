"use strict";(()=>{function i(e){return e.replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;")}function Le(e){return[{key:"details",label:"Details"},{key:"rule",label:"Rule"},{key:"ai-fix",label:"Fix with AI"}].map(s=>`<button class="detail-tab${e===s.key?" active":""}" data-detail-tab="${s.key}">${s.label}</button>`).join("")}function we(e,t,s){let a=e.end_line&&e.end_line!==e.line?`-${e.end_line}`:"",n=He(t,s),l=Fe(t);return`
  ...
  `}function He(e,t){if(e.loadingOptions)return'<div class="detail-loading">Loading AI models\u2026</div>';if(t.length===0)return'<div class="detail-empty">No AI provider is available for the local scanner.</div>';let s=t.find(m=>m.id===e.selectedProviderId)??t[0],a=t.map(m=>`<option value="${i(m.id)}"${e.selectedProviderId===m.id?" selected":""}>${i(m.label)}</option>`).join(""),l=(s?.models??[]).map(m=>`<option value="${i(m)}"></option>`).join(""),c='<div class="ai-fix-helper">This provider can generate local previews without an API key.</div>',p="Required for this provider";s?.requires_api_key&&(s.configured?(c=`<div class="ai-fix-helper">Using the scanner's configured API key. Paste another key below to override it for this session.</div>`,p="Optional override"):c='<div class="ai-fix-helper">Paste an API key for the selected provider to generate the fix.</div>');let d=s?.requires_api_key?`<div class="ai-fix-control-group">
  ...
  `,p="";n.length?p=`
    ...
    `:p='<div class="empty-state compact">No survived mutants match the current filter.</div>',e.innerHTML=l+c+p,xe()}function xe(){let e=document.getElementById("mutant-filter-module"),t=document.getElementById("mutant-sort-field"),s=document.getElementById("mutant-sort-dir");e?.addEventListener("change",a=>{J=a.target.value,W()}),t?.addEventListener("change",a=>{C=a.target.value,W()}),s?.addEventListener("change",a=>{Q=a.target.value,W()})}function Ue(){let e=u.test_signals?.modules??[];if(!e.length)return;let t=o("ts-modules"),s="";for(let l of e){let c=l.health,p=l.coverage,d=l.mutation,g=l.suites??[],v=g.reduce((O,G)=>O+(G.failures??0)+(G.errors??0),0),m=g.reduce((O,G)=>O+(G.tests??0),0),_=l.architecture_role??"",F=_?`<span class="ts-role-badge">${i(_)}</span>`:"",oe=_e(c?.status),Ce=y(p?.coverage),Ie=N(p?.coverage??void 0),Ae=y(d?.changed_code_score??d?.score),Pe=ie(d?.changed_code_score??d?.score);s+=`<div class="ts-module">
    ...
    </div>`,m>0&&(n+=`<div class="sev-segment" style="width:${m/l*100}%;background:${F}" title="${v}: ${m}"></div>`)}o("sev-bars").innerHTML=a,o("sev-proportional").innerHTML=n;let c=se(b,v=>v.type),p=Math.max(1,...Object.values(c)),d={bug:"#ef4444",vulnerability:"#f97316",code_smell:"#22c55e",security_hotspot:"#eab308"},g="";for(let[v,m]of Object.entries(ne)){let _=c[v]??0,F=_/p*100,oe=d[v]??"#64748b";g+=`<div class="sev-bar-row">
    ...
    </div>`}).join(""),o("hotspot-files").querySelectorAll(".hotspot-row").forEach(s=>{s.addEventListener("click",()=>{let a=s.dataset.path;j="grouped",E("issues"),dt(a)})})}function We(){L=(u.test_signals?.modules??[]).flatMap(t=>(t.files??[]).map(s=>Xe(t.name,t.root,s))).filter(t=>t.linesToCover>0).sort((t,s)=>(t.coverage??101)-(s.coverage??101)||s.uncoveredLines.length-t.uncoveredLines.length||t.path.localeCompare(s.path)),!q&&L.length&&(q=L[0].path)}function Xe(e,t,s){let a=s.lines_to_cover??0,n=s.covered_lines??0,l=a>0?n*100/a:null;return{moduleName:e,moduleRoot:t,path:s.path,linesToCover:a,coveredLines:n,coveredLineNumbers:s.covered_line_numbers??[],uncoveredLines:s.uncovered_lines??[],coverage:l}}function Ye(){let e=o("coverage-summary");if(!L.length){e.innerHTML='<div class="empty-state compact">Run with <span class="mono">-with-tests</span> and provide a coverage report to see file-level details.</div>';return}let t=u.test_signals?.summary,s=L.slice(0,5);e.innerHTML=`<div class="coverage-kpis">
    ...
    <div class="coverage-file-list">
      ${s.map(Ze).join("")}
    </div>`,e.querySelectorAll(".coverage-mini-row").forEach(a=>{a.addEventListener("click",()=>{let n=a.dataset.coveragePath;n&&(E("coverage"),he(n))})})}function Ze(e){return`<button class="coverage-mini-row" data-coverage-path="${i(e.path)}">
    ...
    </div>`,e.querySelectorAll(".coverage-row").forEach(t=>{t.addEventListener("click",()=>{let s=t.dataset.coveragePath;s&&he(s)})})}function et(e){return`<button class="coverage-row${e.path===q?" active":""}" data-coverage-path="${i(e.path)}">
...
`).map((p,d)=>{let g=d+1,v=a.has(g),m=!v&&s.has(g),_=st(m,v);return`<div class="code-line${_.stateClass}">
    ...
    <div class="code-surface">${l}</div>
    ...
    </div>`).join("")}function it(){document.querySelectorAll(".tab").forEach(s=>{s.addEventListener("click",()=>{let a=s.dataset.tab;E(a)})});let e=document.getElementById("view-mode-toggle");e&&e.addEventListener("click",()=>{j=j==="flat"?"grouped":"flat",e.textContent=j==="grouped"?"List view":"Group by file",e.setAttribute("aria-pressed",String(j==="grouped")),k()}),document.querySelector(".tabs").addEventListener("keydown",s=>{let a=Array.from(document.querySelectorAll(".tab[role='tab']")),n=a.findIndex(l=>l.getAttribute("aria-selected")==="true");if(s.key==="ArrowRight"){s.preventDefault();let l=a[(n+1)%a.length];l.focus(),E(l.dataset.tab)}else if(s.key==="ArrowLeft"){s.preventDefault();let l=a[(n-1+a.length)%a.length];l.focus(),E(l.dataset.tab)}})}function E(e){Me=e,document.querySelectorAll(".tab").forEach(s=>{s.classList.remove("active"),s.setAttribute("aria-selected","false")});let t=document.querySelector(`.tab[data-tab="${e}"]`);t?.classList.add("active"),t?.setAttribute("aria-selected","true"),document.querySelectorAll(".panel").forEach(s=>s.classList.add("hidden")),o(`panel-${e}`).classList.remove("hidden")}function lt(){let e=[...new Set(b.map(n=>n.rule_key))].sort((n,l)=>n.localeCompare(l)),t=o("filter-rule");e.forEach(n=>{let l=document.createElement("option");l.value=n,l.textContent=n,t.appendChild(l)});let s=new Set;for(let n of b)for(let l of n.tags??[])s.add(l);let a=o("filter-tag");[...s].sort().forEach(n=>{let l=document.createElement("option");l.value=n,l.textContent=n,a.appendChild(l)}),o("filter-severity").addEventListener("change",n=>{P=n.target.value,k()}),o("filter-type").addEventListener("change",n=>{Z=n.target.value,k()}),t.addEventListener("change",n=>{ce=n.target.value,k()}),o("filter-quality").addEventListener("change",n=>{ue=n.target.value,k()}),a.addEventListener("change",n=>{ve=n.target.value,k()}),o("search").addEventListener("input",n=>{pe=n.target.value.toLowerCase(),k()}),Ee()}function Ee(){let e=se(b,s=>s.severity),t=["blocker","critical","major","minor","info"];o("sev-chips").innerHTML=t.map(s=>{let a=e[s]??0,n=R[s]??"#64748b",l=P===s?" active":"";return`<button class="sev-chip${l}" data-sev="${s}"
      ...
      </div>`),s.reference_url&&(a+=`<div class="detail-section">
        <div class="detail-section-title">Reference</div>
        <p><a href="${i(s.reference_url)}" target="_blank" rel="noopener">${i(s.reference_url)}</a></p>
...
Compliant code:
${n.compliant_code}`),n.reference_url&&t.push(`
Reference: ${n.reference_url}`)}}catch{}try{await navigator.clipboard.writeText(t.join(`
  ...
  `;o("detail-body").innerHTML=t,mt(e),ut(e)}function pt(e){let t=R[e.severity]??"#64748b",s=ne[e.type]??e.type,a=e.end_line&&e.end_line!==e.line?`${e.line}:${e.column} \u2013 ${e.end_line}:${e.end_column}`:`${e.line}:${e.column}`,n=`
    ...
    <div class="detail-section">
      ...
      ${e.quality?`<div class="detail-field">
        ...
        <span class="detail-field-value"><span class="quality-badge quality-${i(e.quality)}">${i(Se[e.quality]??e.quality)}</span></span>
      </div>`:""}
      <div class="detail-field">
