...
var _ port.INewCodeRepo = (*NewCodePeriodRepository)(nil)

// GetGlobal returns the global new code period setting.
func (r *NewCodePeriodRepository) GetGlobal(ctx context.Context) (*model.NewCodePeriod, error) {
	return r.getByScope(ctx, "global", nil, nil)
}

// GetForProject returns the project-level setting (without branch).
func (r *NewCodePeriodRepository) GetForProject(ctx context.Context, projectID int64) (*model.NewCodePeriod, error) {
	return r.getByScope(ctx, "project", &projectID, nil)
}

// GetForBranch returns the branch-level setting.
func (r *NewCodePeriodRepository) GetForBranch(ctx context.Context, projectID int64, branch string) (*model.NewCodePeriod, error) {
...
func (r *NewCodePeriodRepository) Resolve(ctx context.Context, projectID int64, branch string) (*model.NewCodePeriod, error) {
	if branch != "" {
...
// DeleteForProject removes a project-level override.
...
func (r *NewCodePeriodRepository) getByScope(ctx context.Context, scope string, projectID *int64, branch *string) (*model.NewCodePeriod, error) {
	ncp := &model.NewCodePeriod{}
	var row pgx.Row
	switch scope {
	case "global":
	...
	case "project":
		row = r.db.Pool.QueryRow(ctx, `
			SELECT id, scope, project_id, branch, strategy, value, created_at, updated_at
			FROM new_code_periods WHERE scope = 'project' AND project_id = $1 LIMIT 1`, *projectID)
	case "branch":
		row = r.db.Pool.QueryRow(ctx, `
			SELECT id, scope, project_id, branch, strategy, value, created_at, updated_at
	...
	err := row.Scan(&ncp.ID, &ncp.Scope, &ncp.ProjectID, &ncp.Branch, &ncp.Strategy, &ncp.Value, &ncp.CreatedAt, &ncp.UpdatedAt)
