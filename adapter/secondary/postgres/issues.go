...
var _ port.IIssueRepo = (*IssueRepository)(nil)

// BulkInsert inserts issues using the PostgreSQL COPY protocol for maximum throughput.
func (r *IssueRepository) BulkInsert(ctx context.Context, issues []model.IssueRow) error {
	if len(issues) == 0 {
		return nil
	}
...
func (r *IssueRepository) Query(ctx context.Context, f model.IssueFilter) ([]*model.IssueRow, int, error) {
	if f.Limit <= 0 {
		f.Limit = 100
	}
	if f.Limit > 1000 {
...
func (r *IssueRepository) Facets(ctx context.Context, projectID, scanID int64) (*model.IssueFacets, error) {
	facets := &model.IssueFacets{
...
func (r *IssueRepository) GetByID(ctx context.Context, id int64) (*model.IssueRow, error) {
	iss := &model.IssueRow{}
	err := r.db.Pool.QueryRow(ctx, `
		SELECT id, scan_id, project_id, rule_key, component_path,
		       line, column_num, end_line, end_column, message,
